...
class ServicioTecnologicoBanner extends Model<BannerAttributes, BannerCreationAttributes> implements BannerAttributes {
  public id!: number; public titulo!: string; public descripcion!: string; public imagen!: string; public activo!: boolean;
}
ServicioTecnologicoBanner.init({
  id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true },
  titulo: { type: DataTypes.STRING(255), allowNull: false },
  descripcion: { type: DataTypes.TEXT, allowNull: true },
  imagen: { type: DataTypes.STRING(255), allowNull: false },
  activo: { type: DataTypes.BOOLEAN, defaultValue: true },
}, { sequelize, tableName: 'servicio_tecnologico_banners', timestamps: true });
export default ServicioTecnologicoBanner;
