...
export const uploadPracticasEstadiasImagen = (req: Request, res: Response, next: NextFunction) => {
  ...
  upload(req, res, (err) => {
    ...
    if (req.file) {
      ...
      (req as any).savedImagePath = `practicas-estadias-banner/${req.file.filename}`;
    }
    next();
  });
};
