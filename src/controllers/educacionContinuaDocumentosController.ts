...
import { EducacionContinuaCategoria, EducacionContinuaDocumento } from '../models/associations';
...
export const obtenerDocumentosPublicos = async (req: Request, res: Response): Promise<void> => {
  try {
    const categorias = await EducacionContinuaCategoria.findAll({
      where: { activo: true },
      order: [['orden', 'ASC']],
      include: [
        {
          model: EducacionContinuaDocumento,
          as: 'documentos',
          where: { activo: true },
...
export const crearCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nueva = await EducacionContinuaCategoria.create({
      titulo,
...
export const actualizarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const categoria = await EducacionContinuaCategoria.findByPk(id);
    if (!categoria) {
...
export const eliminarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await EducacionContinuaDocumento.findAll({ where: { categoria_id: id } });
...
export const obtenerDocumentosAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await EducacionContinuaDocumento.findAll({
      where: whereClause,
      order: [['orden', 'ASC']],
      include: [{ model: EducacionContinuaCategoria, as: 'categoria', attributes: ['titulo'] }]
    });
...
export const crearDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nuevoDoc = await EducacionContinuaDocumento.create({
      titulo,
...
export const actualizarDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documento = await EducacionContinuaDocumento.findByPk(id);
    if (!documento) {
