...
import { FeriasProfesiograficasCategoria, FeriasProfesiograficasDocumento } from '../models/associations';
...
export const obtenerDocumentosPublicos = async (req: Request, res: Response): Promise<void> => {
  try {
    const categorias = await FeriasProfesiograficasCategoria.findAll({
      where: { activo: true },
      order: [['orden', 'ASC']],
      include: [
        {
          model: FeriasProfesiograficasDocumento,
          as: 'documentos',
          where: { activo: true },
...
export const crearCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nueva = await FeriasProfesiograficasCategoria.create({
      titulo,
...
export const actualizarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const categoria = await FeriasProfesiograficasCategoria.findByPk(id);
    if (!categoria) {
...
export const eliminarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await FeriasProfesiograficasDocumento.findAll({ where: { categoria_id: id } });
...
export const obtenerDocumentosAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await FeriasProfesiograficasDocumento.findAll({
      where: whereClause,
      order: [['orden', 'ASC']],
      include: [{ model: FeriasProfesiograficasCategoria, as: 'categoria', attributes: ['titulo'] }]
    });
...
export const crearDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nuevoDoc = await FeriasProfesiograficasDocumento.create({
      titulo,
...
export const actualizarDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documento = await FeriasProfesiograficasDocumento.findByPk(id);
    if (!documento) {
