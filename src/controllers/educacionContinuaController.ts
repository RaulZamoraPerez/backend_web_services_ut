...
export const getVideos = async (req: Request, res: Response) => {
  try {
    const videos = await EducacionContinuaVideo.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(videos);
  } catch (error) {
...
export const createVideo = async (req: Request, res: Response) => {
  try {
    ...
    // Obtener el último orden para agregar al final
    const lastVideo = await EducacionContinuaVideo.findOne({
      order: [['orden', 'DESC']]
    });
...
export const updateVideo = async (req: Request, res: Response) => {
  try {
    ...
    const video = await EducacionContinuaVideo.findByPk(id);
    if (!video) {
...
export const getBanners = async (req: Request, res: Response) => {
  try {
    const banners = await EducacionContinuaBanner.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(banners);
  } catch (error) {
...
export const createBanner = async (req: Request, res: Response) => {
  try {
    ...
    const lastBanner = await EducacionContinuaBanner.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastBanner ? lastBanner.orden + 1 : 0;

    const newBanner = await EducacionContinuaBanner.create({
      titulo: titulo || 'Banner Educación Continua',
      imagen: imagenPath,
      orden: newOrder,
      activo: true
    });
...
export const updateBanner = async (req: Request, res: Response) => {
  try {
    ...
    const banner = await EducacionContinuaBanner.findByPk(id);
    if (!banner) {
...
export const updateBannerOrder = async (req: Request, res: Response) => {
  try {
    ...
    const promises = orderedIds.map((id, index) => {
      return EducacionContinuaBanner.update({ orden: index }, { where: { id } });
    });
