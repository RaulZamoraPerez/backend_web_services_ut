...
import { PortalDocenteCategoria, PortalDocenteDocumento } from '../models/associations';
import path from 'path';
import fs from 'fs';

// Helper for file paths
const UPLOADS_DIR = path.join(__dirname, '../../uploads/portal_docente/documentos');
...
export const obtenerDocumentosPublicos = async (req: Request, res: Response): Promise<void> => {
  try {
    const categorias = await PortalDocenteCategoria.findAll({
      where: { activo: true },
      order: [['orden', 'ASC']],
      include: [
        {
          model: PortalDocenteDocumento,
          as: 'documentos',
          where: { activo: true },
...
export const crearCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nueva = await PortalDocenteCategoria.create({
      titulo,
...
export const actualizarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const categoria = await PortalDocenteCategoria.findByPk(id);
    if (!categoria) {
...
export const eliminarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await PortalDocenteDocumento.findAll({ where: { categoria_id: id } });
...
export const obtenerDocumentosAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await PortalDocenteDocumento.findAll({
      where: whereClause,
      order: [['orden', 'ASC']],
      include: [{ model: PortalDocenteCategoria, as: 'categoria', attributes: ['titulo'] }]
    });
...
export const crearDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nuevoDoc = await PortalDocenteDocumento.create({
      titulo,
...
export const actualizarDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documento = await PortalDocenteDocumento.findByPk(id);
    if (!documento) {
