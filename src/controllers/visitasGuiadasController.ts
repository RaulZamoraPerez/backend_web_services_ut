...
export const getInfo = async (req: Request, res: Response) => {
  try {
    let info = await VisitasGuiadasInfo.findOne();
    if (!info) {
      info = await VisitasGuiadasInfo.create({
        titulo_principal: 'Cursos de Visitas Guiadas',
...
export const updateInfo = async (req: Request, res: Response) => {
  try {
    ...
    if (!info) {
      info = await VisitasGuiadasInfo.create({
        titulo_principal: titulo_principal || 'Cursos de Visitas Guiadas',
...
export const getCursos = async (req: Request, res: Response) => {
  try {
    const cursos = await VisitasGuiadasCurso.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(cursos);
  } catch (error) {
...
export const createCurso = async (req: Request, res: Response) => {
  try {
    ...
    const lastCurso = await VisitasGuiadasCurso.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastCurso ? lastCurso.orden + 1 : 0;

    const newCurso = await VisitasGuiadasCurso.create({
...
export const updateCurso = async (req: Request, res: Response) => {
  try {
    ...
    const curso = await VisitasGuiadasCurso.findByPk(id);
    if (!curso) {
...
export const updateCursoOrder = async (req: Request, res: Response) => {
  try {
    ...
    const promises = orderedIds.map((id, index) => {
      return VisitasGuiadasCurso.update({ orden: index }, { where: { id } });
    });
...
export const getVideos = async (req: Request, res: Response) => {
  try {
    const videos = await VisitasGuiadasVideo.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(videos);
  } catch (error) {
...
export const createVideo = async (req: Request, res: Response) => {
  try {
    ...
    // Obtener el último orden para agregar al final
    const lastVideo = await VisitasGuiadasVideo.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastVideo ? lastVideo.orden + 1 : 0;

    const newVideo = await VisitasGuiadasVideo.create({
      titulo,
      descripcion,
      youtubeId,
      orden: newOrder,
...
export const updateVideo = async (req: Request, res: Response) => {
  try {
    ...
    const video = await VisitasGuiadasVideo.findByPk(id);
    if (!video) {
...
export const getBanners = async (req: Request, res: Response) => {
  try {
    const banners = await VisitasGuiadasBanner.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(banners);
  } catch (error) {
...
export const createBanner = async (req: Request, res: Response) => {
  try {
    ...
    const lastBanner = await VisitasGuiadasBanner.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastBanner ? lastBanner.orden + 1 : 0;

    const newBanner = await VisitasGuiadasBanner.create({
      titulo: titulo || 'Banner Visitas Guiadas',
      imagen: imagenPath,
      orden: newOrder,
      activo: true
    });
...
export const updateBanner = async (req: Request, res: Response) => {
  try {
    ...
    const banner = await VisitasGuiadasBanner.findByPk(id);
    if (!banner) {
...
export const updateBannerOrder = async (req: Request, res: Response) => {
  try {
    ...
    const promises = orderedIds.map((id, index) => {
      return VisitasGuiadasBanner.update({ orden: index }, { where: { id } });
    });
