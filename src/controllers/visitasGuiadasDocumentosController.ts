...
import { VisitasGuiadasCategoria, VisitasGuiadasDocumento } from '../models/associations';
...
export const obtenerDocumentosPublicos = async (req: Request, res: Response): Promise<void> => {
  try {
    const categorias = await VisitasGuiadasCategoria.findAll({
      where: { activo: true },
      order: [['orden', 'ASC']],
      include: [
        {
          model: VisitasGuiadasDocumento,
          as: 'documentos',
          where: { activo: true },
...
export const crearCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nueva = await VisitasGuiadasCategoria.create({
      titulo,
...
export const actualizarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const categoria = await VisitasGuiadasCategoria.findByPk(id);
    if (!categoria) {
...
export const eliminarCategoria = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await VisitasGuiadasDocumento.findAll({ where: { categoria_id: id } });
...
export const obtenerDocumentosAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documentos = await VisitasGuiadasDocumento.findAll({
      where: whereClause,
      order: [['orden', 'ASC']],
      include: [{ model: VisitasGuiadasCategoria, as: 'categoria', attributes: ['titulo'] }]
    });
...
export const crearDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const nuevoDoc = await VisitasGuiadasDocumento.create({
      titulo,
...
export const actualizarDocumento = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const documento = await VisitasGuiadasDocumento.findByPk(id);
    if (!documento) {
