...
export const getInfo = async (req: Request, res: Response) => {
  try {
    let info = await PromocionInstitucionalInfo.findOne();
    if (!info) {
      info = await PromocionInstitucionalInfo.create({
        titulo_principal: 'Cursos de Promoción Institucional',
...
export const updateInfo = async (req: Request, res: Response) => {
  try {
    ...
    if (!info) {
      info = await PromocionInstitucionalInfo.create({
        titulo_principal: titulo_principal || 'Cursos de Promoción Institucional',
...
export const getCursos = async (req: Request, res: Response) => {
  try {
    const cursos = await PromocionInstitucionalCurso.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(cursos);
  } catch (error) {
...
export const createCurso = async (req: Request, res: Response) => {
  try {
    ...
    const lastCurso = await PromocionInstitucionalCurso.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastCurso ? lastCurso.orden + 1 : 0;

    const newCurso = await PromocionInstitucionalCurso.create({
...
export const updateCurso = async (req: Request, res: Response) => {
  try {
    ...
    const curso = await PromocionInstitucionalCurso.findByPk(id);
    if (!curso) {
...
export const updateCursoOrder = async (req: Request, res: Response) => {
  try {
    ...
    const promises = orderedIds.map((id, index) => {
      return PromocionInstitucionalCurso.update({ orden: index }, { where: { id } });
    });
...
export const getVideos = async (req: Request, res: Response) => {
  try {
    const videos = await PromocionInstitucionalVideo.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(videos);
  } catch (error) {
...
export const createVideo = async (req: Request, res: Response) => {
  try {
    ...
    // Obtener el último orden para agregar al final
    const lastVideo = await PromocionInstitucionalVideo.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastVideo ? lastVideo.orden + 1 : 0;

    const newVideo = await PromocionInstitucionalVideo.create({
      titulo,
      descripcion,
      youtubeId,
      orden: newOrder,
...
export const updateVideo = async (req: Request, res: Response) => {
  try {
    ...
    const video = await PromocionInstitucionalVideo.findByPk(id);
    if (!video) {
...
export const getBanners = async (req: Request, res: Response) => {
  try {
    const banners = await PromocionInstitucionalBanner.findAll({
      order: [['orden', 'ASC']]
    });
    res.json(banners);
  } catch (error) {
...
export const createBanner = async (req: Request, res: Response) => {
  try {
    ...
    const lastBanner = await PromocionInstitucionalBanner.findOne({
      order: [['orden', 'DESC']]
    });
    const newOrder = lastBanner ? lastBanner.orden + 1 : 0;

    const newBanner = await PromocionInstitucionalBanner.create({
      titulo: titulo || 'Banner Promoción Institucional',
      imagen: imagenPath,
      orden: newOrder,
      activo: true
    });
...
export const updateBanner = async (req: Request, res: Response) => {
  try {
    ...
    const banner = await PromocionInstitucionalBanner.findByPk(id);
    if (!banner) {
...
export const updateBannerOrder = async (req: Request, res: Response) => {
  try {
    ...
    const promises = orderedIds.map((id, index) => {
      return PromocionInstitucionalBanner.update({ orden: index }, { where: { id } });
    });
