...
export const getBanners = async (req: Request, res: Response) => {
  try {
    const banners = await ServicioTecnologicoBanner.findAll({ order: [['id', 'DESC']] });
    res.json(banners);
  } catch (error) {
    console.error(error);
...
export const createBanner = async (req: Request, res: Response) => {
  try {
    ...
    const banner = await ServicioTecnologicoBanner.create({ titulo, descripcion, imagen: imagenPath, activo: true });
...
export const updateBanner = async (req: Request, res: Response) => {
  try {
    ...
    const banner = await ServicioTecnologicoBanner.findByPk(id);
