...
/**
 * Script único para poblar toda la base de datos con datos de prueba
...
// Definir modelos básicos para el seed
const Carrera = sequelize.define('Carrera', {
  ...
  perfil_ingreso: { type: DataTypes.TEXT, allowNull: false },
  perfil_egreso: { type: DataTypes.TEXT, allowNull: false },
  campo_laboral: { type: DataTypes.TEXT, allowNull: false },
...
async function seedCarreras() {
  try {
    ...
    console.log('🎉 Carreras pobladas exitosamente');
  } catch (error) {
    console.error('❌ Error poblando carreras:', error);
    throw error;
  }
}
...
async function seedHeroSlides() {
  try {
    console.log('🌱 Poblando hero slides...');
...
async function seedEvento() {
  try {
    console.log('🌱 Poblando evento...');

    // Crear fecha futura (15 días desde hoy)
...
async function seedAll() {
  try {
    ...
    console.log('   • 4 hero slides creados');
    console.log('   • 1 evento creado');
